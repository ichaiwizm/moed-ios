...
//  `UserDefaults.standard` de la cible. Rien ne quitte l'appareil. Le widget,
//  compilé avec ce même fichier (double target membership), recalcule tout
//  localement via les moteurs déterministes offline — aucun conteneur partagé
//  n'est requis (les moteurs sont 100 % offline et déterministes).
//
...
/// `moed_settings` dans `UserDefaults.standard`. Le format `Data`
/// (plutôt que des clés éclatées) garantit l'atomicité et suit l'évolution du
/// type `Settings` sans migration de clés.
enum SettingsStore {

    /// Clé de stockage dans `UserDefaults`.
    private static let key = "moed_settings"

    /// `UserDefaults.standard` de la cible (app ou widget). Pas de conteneur
    /// partagé : les moteurs déterministes offline recalculent tout localement.
